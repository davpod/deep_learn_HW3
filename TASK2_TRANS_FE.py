...
class CharTransformerEncoder(nn.Module):
    def __init__(self, vocab_size, embed_dim=64, nhead=4, hidden_dim=128, num_layers=2, dropout=0.2):
        ...
        # Use batch_first=True to avoid permute
    ...
    def forward(self, x):
        # Ensure input is on same device as embedding
        x = x.to(self.emb.weight.device)
        x = self.emb(x)           # [B, L, E]
        x = self.transformer(x)   # [B, L, E]
        x = x.permute(0, 2, 1)    # [B, E, L] for pooling
...
class SiameseTransformerRegression(nn.Module):
    def __init__(self, vocab_size, embed_dim=64, nhead=4, hidden_dim=128, num_layers=2, dropout=0.2):
        super().__init__()
        self.encoder = CharTransformerEncoder(vocab_size, embed_dim, nhead, hidden_dim, num_layers, dropout)
        self.fc = nn.Linear(128*2, 1)

    def forward(self, s, t):
        s, t = s.to(self.encoder.emb.weight.device), t.to(self.encoder.emb.weight.device)
        es = self.encoder(s)
        et = self.encoder(t)
...
# ======================
feature_model = SiameseTransformerRegression(len(char2idx), embed_dim=32, nhead=2, hidden_dim=64, num_layers=2, dropout=DROPOUT)
feature_model.load_state_dict(torch.load("char_siamese_TRANS_model_state.pth"))
feature_model.eval()
...
# ======================
# OPTION 2: Classical ML
# ======================
# Ridge
start_time_ridge = time.time()
ridge = Ridge(alpha=1.0)
ridge.fit(train_emb.numpy(), train_labels.numpy())
pred_ridge = ridge.predict(test_emb.numpy())
ridge_rmse = np.sqrt(mean_squared_error(test_labels.numpy(), pred_ridge))
ridge_mae = mean_absolute_error(test_labels.numpy(), pred_ridge)
ridge_runtime = time.time() - start_time_ridge
print(f"Ridge | Test RMSE: {ridge_rmse:.4f} | MAE: {ridge_mae:.4f} | Runtime: {ridge_runtime:.2f}s")
